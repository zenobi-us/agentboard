...
export async function appendSourceSnapshot(
  ...
  try {
    previousBoundary = await readFile(boundaryPath, "utf8");
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
  }
  await appendFile(boundaryPath, `${JSON.stringify({ snapshot_key: snapshotKey, snapshot_id: snapshotId })}\n`);
  if (cancellation.aborted) {
    if (previousBoundary) await writeFile(boundaryPath, previousBoundary);
    else await unlink(boundaryPath).catch(() => undefined);
    throwIfCancelled(cancellation);
  }
}
