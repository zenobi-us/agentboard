...
export function runtime(config: GithubSource, context: { sourceId: string; cancellation: AbortSignal }): SourceRuntime {
  return { collect: async () => {
    ...
    const query = config.query.split(/\s+/).includes("is:issue") ? config.query : `is:issue ${config.query}`;
    const items: Item[] = [];
    ...
    for (let page = 1; items.length < (config.limit ?? 50); page += 1) {
      ...
      const response = await fetch(`https://api.github.com/search/issues?q=${encodeURIComponent(query)}&per_page=${pageSize}&page=${page}`, { headers: { authorization: `Bearer ${token}`, accept: "application/vnd.github+json", "X-GitHub-Api-Version": "2022-11-28", "user-agent": "agentboard" }, signal: context.cancellation });
...
export async function healthCheck(config: GithubSource, context: { sourceId: string; cancellation: AbortSignal }): Promise<void> { await helper(config.credentials.helper, context.cancellation); }
