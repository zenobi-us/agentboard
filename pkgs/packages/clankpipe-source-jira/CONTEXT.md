# ClankPipe Jira Source Context

`clankpipe-source-jira` collects Jira issues through Jira's REST API and normalizes them into ClankPipe Items.
...
**JQL**:
The query string sent to Jira. ClankPipe does not parse it.
_Avoid_: ClankPipe query
...
- Jira query semantics belong to Jira. ClankPipe must not reinterpret JQL.
...
- Jira's internal issue id is `item.id`; the issue key or `field_map.id` value is `item.reference_id`.
