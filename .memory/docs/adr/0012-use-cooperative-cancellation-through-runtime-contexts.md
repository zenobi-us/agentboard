...
AgentBoard uses one invocation-scoped cancellation signal, created by the CLI composition root and passed through runtime and health-check contexts. The Rust runtime uses `tokio_util::sync::CancellationToken`. The Bun runtime uses `AbortSignal`.
